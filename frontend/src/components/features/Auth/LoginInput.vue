<template>
  <form @submit.prevent="handleSubmit" class="w-full">
    ...
    <div class="flex flex-row items-center justify-between mb-6">
      <label class="flex items-center gap-2 cursor-pointer text-sky-950 font-semibold">
        <input
          type="checkbox"
          v-model="rememberMe"
          class="w-4 h-4 border-2 border-gray-300 rounded accent-sky-950 cursor-pointer hover:accent-sky-800 transition-colors"
        />
        <span class="select-none">Ingat Saya</span>
      </label>
      <RouterLink
        to="/lupa-password"
        class="text-orange-400 font-semibold cursor-pointer hover:text-orange-500 transition-colors"
      >
        Lupa password?
      </RouterLink>
    </div>
